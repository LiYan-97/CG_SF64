...
def load_data():
    ...
    path_df = pd.read_csv('input_path_80_8.csv', encoding='gbk')
    float_df = pd.read_csv('input_path_float_80_8.csv', encoding='gbk')
...
def output_results(sess, data, feed, graph):
    ...
    ozone_df.to_csv('output_ozone_80_8.csv', index=None)
    ...
    od_df.to_csv('output_od_80_8.csv', index=None)
    ...
    path_df.to_csv('output_path_80_8.csv', index=None)
    ...
    link_df.to_csv('output_link_80_8.csv', index=None)
    ...
    cross_df.to_csv('output_cross_80_8.csv', index=None)
...
with tf.Session() as sess:
    ...
    for nb_iter in range(maximum_iterations):
        ...
        dataframe.to_csv("output_loss_80_8.csv", index=False)
    ...
    plt.savefig('convergence curv with 5F(have cross layer)_80_8.png', dpi=800, format='png')
    plt.show()
...
# 在带有cross的5F中，只训练4个F（F总=0.25*F1+0.25*F2+0.25*F3+0.25*F4）：
    ...
    plt.yticks(fontproperties = 'Times New Roman', size = 12)
    # plt.savefig('convergence curv.svg', dpi=300, format='svg')
    plt.savefig('convergence curv with 5F_4(have cross layer)_80_8.png', dpi=800, format='png')
    plt.show()
