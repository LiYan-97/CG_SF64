...
def load_data():
    ...
    link_df = pd.read_csv('input_link_20_3.csv', encoding='gbk')
    sensor_df = pd.read_csv('input_link_sensor_20_3.csv', encoding='utf-8')
...
def output_results(sess, data, feed, graph):
    ...
    ozone_df.to_csv('output_ozone_20_3_link.csv', index=None)
    ...
    od_df.to_csv('output_od_20_3_link.csv', index=None)
    ...
    path_df.to_csv('output_path_20_3_link.csv', index=None)
    ...
    link_df.to_csv('output_link_20_3_link.csv', index=None)
    ...
    cross_df.to_csv('output_cross_20_3_link.csv', index=None)
...
# lr_mobile = tf.compat.v1.train.exponential_decay(0.00005, curr_iter, decay_steps=maximum_iterations/2, decay_rate=0.5)
# lr_float  = tf.compat.v1.train.exponential_decay(0.0000008, curr_iter, decay_steps=maximum_iterations/5, decay_rate=0.5)
# lr_count = tf.compat.v1.train.exponential_decay(0.000003, curr_iter, decay_steps=maximum_iterations/20, decay_rate=0.5)
# lr_video  = tf.compat.v1.train.exponential_decay(0.00000003, curr_iter, decay_steps=maximum_iterations, decay_rate=0.5)
# learning_rate = tf.compat.v1.train.exponential_decay(0.0005, curr_iter, decay_steps=maximum_iterations/20, decay_rate=0.5)
...
with tf.Session() as sess:
    ...
    for nb_iter in range(maximum_iterations):
        ...
        dataframe.to_csv("output_loss_20_3_link.csv", index=False)
    ...
    plt.savefig('convergence curv with 5F(have cross layer)_20_3_link.png', dpi=800, format='png')
    plt.show()
...
# 在带有cross的5F中，只训练4个F（F总=0.25*F1+0.25*F2+0.25*F3+0.25*F4）：
    ...
    plt.yticks(fontproperties = 'Times New Roman', size = 12)
    # plt.savefig('convergence curv.svg', dpi=300, format='svg')
    plt.savefig('convergence curv with 5F_4(have cross layer)_20_3_link.png', dpi=800, format='png')
    plt.show()
