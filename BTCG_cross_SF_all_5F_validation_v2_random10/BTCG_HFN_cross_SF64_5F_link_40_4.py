...
def load_data():
    ...
    link_df = pd.read_csv('input_link_40_4.csv', encoding='gbk')
    sensor_df = pd.read_csv('input_link_sensor_40_4.csv', encoding='utf-8')
...
def output_results(sess, data, feed, graph):
    ...
    ozone_df.to_csv('output_ozone_40_4_link.csv', index=None)
    ...
    od_df.to_csv('output_od_40_4_link.csv', index=None)
    ...
    path_df.to_csv('output_path_40_4_link.csv', index=None)
    ...
    link_df.to_csv('output_link_40_4_link.csv', index=None)
    ...
    cross_df.to_csv('output_cross_40_4_link.csv', index=None)
...
with tf.Session() as sess:
    ...
    for nb_iter in range(maximum_iterations):
        ...
        dataframe.to_csv("output_loss_40_4_link.csv", index=False)
    ...
    plt.savefig('convergence curv with 5F(have cross layer)_40_4_link.png', dpi=800, format='png')
    plt.show()
...
# 在带有cross的5F中，只训练4个F（F总=0.25*F1+0.25*F2+0.25*F3+0.25*F4）：
    ...
    plt.yticks(fontproperties = 'Times New Roman', size = 12)
    # plt.savefig('convergence curv.svg', dpi=300, format='svg')
    plt.savefig('convergence curv with 5F_4(have cross layer)_40_4_link.png', dpi=800, format='png')
    plt.show()
