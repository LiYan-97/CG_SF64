...
def output_results(sess, data, feed, graph):
    ...
    cross_df.to_csv('output_cross.csv', index=None)
...
with tf.Session() as sess:
    ...
    plt.savefig('convergence curv with 5F(have cross layer).png', dpi=800, format='png')
    plt.show()
...
# 在带有cross的5F中，只训练4个F（F总=0.25*F1+0.25*F2+0.25*F3+0.25*F4）：
    ...
    plt.yticks(fontproperties = 'Times New Roman', size = 12)
    # plt.savefig('convergence curv.svg', dpi=300, format='svg')
    plt.savefig('convergence curv with 5F_4(have cross layer).png', dpi=800, format='png')
    plt.show()
