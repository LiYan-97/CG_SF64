...
def load_data(): 
    ...
    node_df['is_zone'] = node_df.apply(lambda x: True if x.zone_id!=-1 else False, axis=1)   # 交通小区起终点不等于-1
...
#        path.path_flow = tf.multiply(path.rou, path.belonged_od.estimate_OD_flow)
    ...
    loss = tf.reduce_sum(tf.stack([0.25*f_survey, 0.25*f_mobile, 0.25*f_float, 0.25*f_count]),name='loss/total_sum')
...
#
# lr_survey = tf.compat.v1.train.exponential_decay(200.0, curr_iter, decay_steps=maximum_iterations, decay_rate=0.5)
# lr_mobile = tf.compat.v1.train.exponential_decay(0.0000004, curr_iter, decay_steps=maximum_iterations/5, decay_rate=0.5)
# lr_float = tf.compat.v1.train.exponential_decay(0.0000002, curr_iter, decay_steps=maximum_iterations/5, decay_rate=0.5)
# lr_count = tf.compat.v1.train.exponential_decay(0.00004, curr_iter, decay_steps=maximum_iterations/2, decay_rate=0.005)
# learning_rate = tf.compat.v1.train.exponential_decay(0.0005, curr_iter, decay_steps=maximum_iterations, decay_rate=0.5)

# lr_survey = tf.compat.v1.train.exponential_decay(250.0, curr_iter, decay_steps=maximum_iterations, decay_rate=0.5)
...
with tf.Session() as sess:
    ...
    for nb_iter in range(maximum_iterations):
        ...
        train_survey, _ = sess.run([f_survey, opt_survey], feed_dict=feed) #### 原来的f_survey和新的（opt_survey）相减，求GAP值，也就是train
    ...
    t1 = datetime.datetime.now()
    plt.tight_layout()
    plt.plot(list_survey, 'palegreen', label="F1-(survey)")
    plt.plot(list_mobile, 'moccasin', label="F2-(mobile)")
    plt.plot(list_float, 'cornflowerblue', label="F3-(OBD)")
    plt.plot(list_sensor, 'lightcoral', label="F4-(monitoring_link)")
    plt.plot(list_total, 'lightsteelblue', linestyle='--', linewidth='2.5', label="F-(total)")
    # plt.plot(list_survey, 'r-')
    # plt.plot(list_mobile, 'b-')
    # plt.plot(list_float, 'g-')
    ...
    # plt.title('Convergence Curve(with no cross layer)', pad=12, fontdict={'family': 'Times New Roman', 'weight': 'bold', 'size': 20})  # 改变图标题字体,family用于设置字体
    # plt.xlabel('iteration', labelpad=7.5, fontdict={'family': 'Times New Roman', 'weight': 'bold', 'size': 14, 'color': 'dimgrey'})  # 改变横坐标轴标题字体  labelpad=10表示坐标标题距离坐标轴的垂直距离
    ...
    # plt.savefig('convergence curv.svg', dpi=300, format='svg')
    plt.savefig('convergence curv with 4F(have no cross layer).png', dpi=300, format='png')
    plt.show()
